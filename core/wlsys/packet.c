...
#else
		newskb = dev_alloc_skb( *(mib->mib_amsdu_maxsize));
		if(newskb)
		{
#ifdef WL_KERNEL_26
    								...
    								else  /** Out of Seq **/
    								{
    									...
    									if(wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[tid]<SeqNo) /** Valid case **/
    									{
    										if(SeqNo-wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[tid]>(MAX_AMPDU_REORDER_BUFFER-1))
    										{
    											...
    											wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[tid]=SeqNo+1;  /**  assuming next pck **/
    											goto blkackoutcontinue;
    										}
    							...
    							else /** reordering has happen **/
    							{
    								...
    								*****************************************************************************************************************/
    								...
    								else  /** SeqNo not equal **/
    								{
    									...
    									if(jiffies - wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].Time[tid] > 100)
    									{
#ifdef AMPDU_DEBUG
    										...
    										Ampdu_Flush_All_Pck_in_Reorder_queue(dev,pStaInfo->Aid, tid);
    										wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].ReOrdering[tid]=FALSE;
    										wlpptr->wlpd_p->AmpduPckReorder[pStaInfo->Aid].CurrentSeqNo[tid]=SeqNo+1;  /** hack, assuming next pck **/
    										goto blkackoutcontinue;
    									}
...
blkackoutcontinue:
	...
	case IEEE_TYPE_MANAGEMENT:
		...
		{
			vmacApInfo_t * vmactem_p; 
			vmactem_p = vmacGetMBssByAddr(vmacSta_p, (UINT8 *)&(wh->addr3));
#ifdef WDS_FEATURE            
