...
#ifdef RXPATHOPT
	if ((StaInfo_p->ClientMode == GONLY_MODE) || (StaInfo_p->ClientMode == AONLY_MODE))
	{
		printk("RxPathOpt = %d\n",*(wlpptrSta->vmacSta_p->Mib802dot11->mib_RxPathOpt));
		wlFwSetRxPathOpt(pStaDev, *(wlpptrSta->vmacSta_p->Mib802dot11->mib_RxPathOpt));
	}
#endif    
	return;
}	
