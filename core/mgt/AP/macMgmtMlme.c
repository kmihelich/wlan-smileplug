...
/* -KM	WLSYSLOG(vmacSta_p->dev,WLSYSLOG_CLASS_ALL, WLSYSLOG_MSG_MLME_DEAUTH_TOSTA "%02x%02x%02x%02x%02x%02x Reason %d\n", ((unsigned char *)Addr)[0],
...
///////////////////////////////////////////////////////////////////////////////
#ifdef WMM_PS_SUPPORT
void macMgmtMlme_UpdatePwrMode(vmacApInfo_t *vmacSta_p,IEEEtypes_GenHdr_t *Hdr_p, extStaDb_StaInfo_t *pStaInfo, UINT8 Prio)
#else
void macMgmtMlme_UpdatePwrMode(vmacApInfo_t *vmacSta_p,IEEEtypes_GenHdr_t *Hdr_p, extStaDb_StaInfo_t *pStaInfo)
#endif
{
...
#ifdef WMM_PS_SUPPORT
			NotifyPwrModeChange(&Hdr_p->Addr2, PWR_MODE_PWR_SAVE,0,Prio);
#else
			NotifyPwrModeChange(&Hdr_p->Addr2, PWR_MODE_PWR_SAVE);
#endif
			vmacSta_p->PwrSaveStnCnt++;
		}
	}
	else
	{
		if (Hdr_p->FrmCtl.PwrMgmt == 0)
		{
			...
			/* Inform the transmit module */
#ifdef WMM_PS_SUPPORT
			NotifyPwrModeChange(&Hdr_p->Addr2, PWR_MODE_ACTIVE,0,Prio);
#else
			NotifyPwrModeChange(&Hdr_p->Addr2, PWR_MODE_ACTIVE);
#endif
			if (vmacSta_p->PwrSaveStnCnt)
				vmacSta_p->PwrSaveStnCnt--;
		}
