...
#if 1 //bt_port
UINT32 ReceiveDataCount[NUM_OF_WLMACS];
#endif
...
/* Imported customer specific function */
/* To be customised by customer at aproot.c */
UINT32 g_CheckStatusTime[NUM_OF_WLMACS] = {30, 30};
UINT32 g_PrbeReqCheckTheshold[NUM_OF_WLMACS] = {3, 3};
...
extern void syncSrv_StatusCheckTimeOut(UINT8 *data)
{
    ...
    if(vStaInfo_p->AssociatedFlag)
    {
        ...
        {
            if (ReceiveDataCount[macIndex] == 0)
            {
                /* Infrastructure Mose */
                /* Check for link lost */
...
{  
   ...
   mlmeApiSetRfChannel(vStaInfo_p, vStaInfo_p->PreScanRfChannel, 1, TRUE);
   /* Restore the state */
