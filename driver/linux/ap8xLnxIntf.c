...
UINT8   tmpScanResults[MAX_SCAN_BUF_SIZE];
UINT8   tmpNumScanDesc=0;
#endif
#ifdef MBSS
...
static int __init wlmodule_init(void)
{
	...
	memcpy(wldriver.name, DRV_NAME, sizeof(DRV_NAME));
...
int	wlInit(struct net_device *netdev, u_int16_t devid)
{
	...
	writel((wlpptr->wlpd_p->descData[1].pPhysTxRing),
		wlpptr->ioBase0 + wlpptr->wlpd_p->descData[1].wcbBase);
	writel((wlpptr->wlpd_p->descData[2].pPhysTxRing),
		wlpptr->ioBase0 + wlpptr->wlpd_p->descData[2].wcbBase);
	writel((wlpptr->wlpd_p->descData[3].pPhysTxRing),
		wlpptr->ioBase0 + wlpptr->wlpd_p->descData[3].wcbBase);
#endif
...
#else
		if(wlInit_client(wlpptr, &wlpptr->hwData.macAddr[0], &wlpptr->hwData.macAddr[0]))
#endif
		{
			printk("*********** Fail to Init Client \n");
		}
...
#endif
{
	...
	do
	{
		...
		if ((intStatus & ISR_SRC_BITS) || (currIteration < MAX_ISR_ITERATION))
		{
			...
			if (intStatus & MACREG_A2HRIC_BIT_MIC_ERROR)
			{
				intStatus &= ~MACREG_A2HRIC_BIT_MIC_ERROR;
				WLDBG_INFO(DBG_LEVEL_2, "MIC Error MACREG_A2HRIC_BIT_MIC_ERROR. \n");
...
int wlreset(struct net_device *netdev)
{
	...
	*/
...
#ifdef COEXIST_20_40_SUPPORT
	{
		...
		if((*(mib->USER_ChnlWidth )&0xf0) && (*(vmacSta_p->Mib802dot11->mib_ApMode) < AP_MODE_A_ONLY))
		{
...
#if NUM_OF_DESCRIPTOR_DATA >3
	writel(0x00000000, wlpptr->ioBase0 + wlpptr->wlpd_p->descData[1].wcbBase);
	writel(0x00000000, wlpptr->ioBase0 + wlpptr->wlpd_p->descData[2].wcbBase);
	writel(0x00000000, wlpptr->ioBase0 + wlpptr->wlpd_p->descData[3].wcbBase);
#endif
...
#endif
		{
			...
			if (vmacSta_p->gUserInitScan == TRUE)
			{
				...
				if (smeGetScanResults(vmacEntry_p->phyHwMacIndx, &numDescpt, &bufSize, &buf_p) == MLME_SUCCESS )
				{
					if (numDescpt > 0)
					{
						memset(tmpScanResults, 0, MAX_SCAN_BUF_SIZE);
						memcpy(tmpScanResults, buf_p, bufSize);
						tmpNumScanDesc = numDescpt;
					}
				}
