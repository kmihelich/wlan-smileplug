...
int wlFwGetHwSpecs(struct net_device *netdev)
{
	...
	wlpptr->wlpd_p->descData[1].wcbBase       = ENDIAN_SWAP32(pCmd->WcbBase1)   & 0x0000ffff;
	wlpptr->wlpd_p->descData[2].wcbBase       = ENDIAN_SWAP32(pCmd->WcbBase2)   & 0x0000ffff;
	wlpptr->wlpd_p->descData[3].wcbBase       = ENDIAN_SWAP32(pCmd->WcbBase3)   & 0x0000ffff;
#endif
...
#if NUM_OF_DESCRIPTOR_DATA >3
	pCmd->WcbBase[1]	 = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[1].pPhysTxRing);//ENDIAN_SWAP32(	wlpptr->descData[1].wcbBase )	& 0x0000ffff;
	pCmd->WcbBase[2]  = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[2].pPhysTxRing);//ENDIAN_SWAP32(wlpptr->descData[2].wcbBase )	& 0x0000ffff;
	pCmd->WcbBase[3]  = ENDIAN_SWAP32(wlpptr->wlpd_p->descData[3].pPhysTxRing);//ENDIAN_SWAP32(	wlpptr->descData[3].wcbBase)	& 0x0000ffff;
#endif
...
int wlFwGetHwStats(struct net_device *netdev, char *page)
{
	...
	if(p)
	{
		...
		p += sprintf(p,"FCSErrorCount....................%10u\n", ENDIAN_SWAP32((int)pCmd->FCSErrorCount));
...
int wlFwSetChannelSwitchIE(struct net_device *netdev, UINT32 nextChannel, UINT32 mode, UINT32 count, CHNL_FLAGS Chanflag)
{
	...
	if(pCmd->ChannelFlags.ChnlWidth == CH_AUTO_WIDTH)
...
int wlFwSetIEs(struct net_device *netdev)
{
	...
	if (*(mib->QoSOptImpl))
	{
		...
		pCmd->IeListLen  += AddHT_IE(wlpptr->vmacSta_p,(IEEEtypes_HT_Element_t*)&(pCmd->IeList[retval]));
		retval = pCmd->IeListLen;	
...
#ifdef INTOLERANT40
		{
			...
			pCmd->IeListLen  += AddChanReport_IE(wlpptr->vmacSta_p,(IEEEtypes_ChannelReportEL_t*)&(pCmd->IeList[retval]));
			retval = pCmd->IeListLen;	
		}
#endif
		pCmd->IeListLen += AddAddHT_IE(wlpptr->vmacSta_p,(IEEEtypes_Add_HT_Element_t*)&(pCmd->IeList[retval]));
		retval = pCmd->IeListLen;	


#ifdef COEXIST_20_40_SUPPORT
		/** We are only going to use 20/40 coexist for 2.4G band **/
		if(*(vmacSta_p->ShadowMib802dot11->mib_HT40MIntoler)  &&  !(*(vmacSta_p->Mib802dot11->mib_ApMode) >= AP_MODE_A_ONLY 
			|| *(vmacSta_p->Mib802dot11->mib_ApMode) <= AP_MODE_MIXED))
		{
#if 0 //optional IE, not require at this time
			{
				...
				pCmd->IeListLen  += Add20_40_Coexist_IE(wlpptr->vmacSta_p,(IEEEtypes_20_40_BSS_COEXIST_Element_t *)&(pCmd->IeList[retval]));
				retval = pCmd->IeListLen;	
			}
			{
				...
				pCmd->IeListLen  += Add20_40Interant_Channel_Report_IE(wlpptr->vmacSta_p,(IEEEtypes_20_40_INTOLERANT_CHANNEL_REPORT_Element_t *)&(pCmd->IeList[retval]));
				retval = pCmd->IeListLen;	
			}
#endif
			{
				...
				pCmd->IeListLen  += AddOverlap_BSS_Scan_Parameters_IE(wlpptr->vmacSta_p,(IEEEtypes_OVERLAP_BSS_SCAN_PARAMETERS_Element_t  *)&(pCmd->IeList[retval]));
				retval = pCmd->IeListLen;	

			}
			{
				extern UINT16 AddExtended_Cap_IE(vmacApInfo_t *vmacSta_p,IEEEtypes_Extended_Cap_Element_t * pNextElement);
				pCmd->IeListLen  += AddExtended_Cap_IE(wlpptr->vmacSta_p,(IEEEtypes_Extended_Cap_Element_t *)&(pCmd->IeList[retval]));
				retval = pCmd->IeListLen;	
			}
		}
...
#ifdef INTEROP
		retval = pCmd->IeListLen;
		pCmd->IeListLen  +=Add_Generic_HT_IE(wlpptr->vmacSta_p,(IEEEtypes_Generic_HT_Element_t* )&(pCmd->IeList[retval]));
		retval = pCmd->IeListLen;
		pCmd->IeListLen  +=Add_Generic_AddHT_IE(wlpptr->vmacSta_p,(IEEEtypes_Generic_Add_HT_Element_t*)&(pCmd->IeList[retval]));
		/** For I_COMP only **/
		retval = pCmd->IeListLen;
#endif
	...
	{
		//add M IE
		pCmd->IeListLen  += AddM_IE(wlpptr->vmacSta_p,(IEEEtypes_HT_Element_t*)&(pCmd->IeList[retval]));
		retval = pCmd->IeListLen;	
	}

	{
		...
		if (*(mib->mib_RptrMode))	
		{
			pCmd->IeListLen  += AddM_Rptr_IE(wlpptr->vmacSta_p,(IEEEtypes_HT_Element_t*)&(pCmd->IeList[retval]));
			retval = pCmd->IeListLen;	
		}
	}
...
static void wlsendCommand(struct net_device *netdev)
{
	...
	dummy = readl(wlpptr->ioBase1 + MACREG_REG_INT_CODE);
...
static BOOLEAN AnyDevAmsduEnabled(struct net_device *netdev)
{
	...
	MIB_802DOT11 *mib;
	int i=0;
	while(wlpptr->vdev[i] && i <=MAX_VMAC_INSTANCE_AP )
	{
...
int wlFwGetRateTable(struct net_device *netdev, UINT8 *addr)	  
{
	struct wlprivate *wlpptr = NETDEV_PRIV_P(struct wlprivate, netdev);
	...
	if(!retval)
		memcpy(&wlpptr->calTbl, &pCmd->SortedRatesIndexMap, 2*RATE_TBL_SIZE);
